...
- **Lightroom hangs on "performing shutdown tasks" when quitting**, or warns that "a shutdown task for the plug-in became unresponsive" — `LrShutdownApp` requires the shutdown script to return a table with an `LrShutdownFunction` that calls Lightroom's `doneFunction`. Without it Lightroom waits out its 10 s no-progress timeout on every quit. The script now returns that table, raises the stop flag, and reports done at once; the background loop sees the flag within 0.2 s and releases both ports on its way out. A handler that is already running (a large search, export, import, or preset apply) must still finish before Lightroom can quit.
